...
export default function HomePage() {
    ...
    <>
      <div className="flex items-center justify-between gap-2 px-1.5 pt-1">
        <div className="min-w-0">
          <h1 className="truncate text-[17px] font-semibold tracking-tight">AyeTab</h1>
          ...
          {prefs.recents.slice(0, 5).map((id) => {
            ...
            return (
              <button
                ...
                className="truncate rounded-xl px-2.5 py-1.5 text-left text-[12px] text-muted-foreground transition-[background-color,color,transform] duration-100 ease-out-strong active:scale-[0.98] [@media(hover:hover)_and_(pointer:fine)]:hover:bg-black/[0.04] [@media(hover:hover)_and_(pointer:fine)]:hover:text-foreground dark:[@media(hover:hover)_and_(pointer:fine)]:hover:bg-white/[0.06]"
              >
                {tool.name}
              </button>
            );
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <aside className="material-sidebar sticky top-0 hidden h-screen w-[232px] shrink-0 flex-col gap-4 px-3 py-5 md:flex">
        {sidebar}
      </aside>
      ...
      <main className="flex min-w-0 flex-1 flex-col overflow-auto">
        <div className="sticky top-0 z-10 flex items-center gap-3 px-4 py-3 backdrop-blur-xl md:hidden">
          <button
            type="button"
          ...
          <p className="text-[15px] font-semibold tracking-tight">AyeTab</p>
        </div>

        <div className="mx-auto flex w-full max-w-3xl flex-col gap-5 px-5 py-6 md:px-10 md:py-10">
